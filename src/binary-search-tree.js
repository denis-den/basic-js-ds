...
class BinarySearchTree {
  ...
  add(data) {
    ...
    function addTreeNode(item, data) {
      if (!item) { return new Node(data); }
      if (item.data === data) { return item; }
      if (data < item.data) { item.left = addTreeNode(item.left, data); } 
      else { item.right = addTreeNode(item.right, data); }
      return item;
    }
  }

  has(data) { return !!this.find(data); }
